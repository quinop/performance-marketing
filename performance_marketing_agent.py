...
def run_agent():
    ...
    # Prompt updated for Discord Markdown
    ad_summary = get_summary_safe(
        ad_items, 
        "Summarize updates for Meta, Google, TikTok, or Apple Ads. "
        "Use bullet points. Format: **Platform**: Summary (1 sentence)."
    )

    tool_summary = get_summary_safe(
        tool_items, 
        "Summarize updates for ASO tools (AppTweak, Sensor Tower) or industry shifts. "
        "Use bullet points. Format: **Tool**: Summary (1 sentence)."
    )
